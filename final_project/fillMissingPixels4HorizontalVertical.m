...
% non-missing pixels surrounding it. It first uses vertical and horizontal
% distance 1 to find non-missing pixels. If they is none, it checks surrounding
% pixels at distance 2 and so on. Finally, it stops at distance specified by
% the parameter `max_distance`. A value of 0.5 will be used to fill the missing
% pixels if all surrounding pixels of distance up to `max_distance` are
% all missing.
%
% Input:
...
function img_filled = fillMissingPixels4HorizontalVertical(img, msk, max_dist)
