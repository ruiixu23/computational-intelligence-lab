...
% is, every missing pixels in that patch is fill with the mean value of the
...
function img_filled = fillMissingPixelsPatchMean(img, msk, patch_size)
    img_filled = img;
    ...
    for patch_row_start = 1:patch_size:size(img, 1)
        for patch_column_start = 1:patch_size:size(img, 2)
            ...
            if (nnz_count > 0 && nnz_count < patch_size * patch_size)
                total = 0;
                for i = 1:nnz_count
                    total = total + patch(non_missing_pixels_idx(i));
                end
                patch(~patch_msk) = total / nnz_count;
