...
% non-missing pixels surrounding it. It first uses horizontal, vertial
% and diagonal distance 1 to check the non-missing pixels. If they
% are all missing, it checks surrounding pixels at distance 2 and so on.
% Finally, it stops at distance specified by the parameter `max_distance`. Note
...
function img_filled = fillMissingPixels8Frame(img, msk, max_dist)
    ...
    for dist = 1:max_dist
        neighbours{1, dist} = [dist ...
            (-1 * dist) ...
            (dist * img_padded_height) ...
            (-1 * dist * img_padded_height) ...
            (dist + dist * img_padded_height) ...
...
%                 (dist + dist * img_padded_height + idx) ...
