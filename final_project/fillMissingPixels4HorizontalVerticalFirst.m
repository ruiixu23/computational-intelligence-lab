...
% non-missing pixels surrounding it. It first uses horizontal and vertical
% distance 1 to find non-missing pixels. If they is none, it checks surrounding
% pixels at diagonal distance 1 and so on. Finally, it stops at distance
% specified by the parameter `max_distance`. A value of 0.5 will be used
% to fill the missing pixels if all surrounding pixels of distance
% up to `max_distance` are all missing.
%
% Input:
...
function img_filled = fillMissingPixels4HorizontalVerticalFirst(img, msk, max_dist)
...
%     neighbours_diagonal = cell(1, max_dist);
%     neighbours_horizontal = cell(1, max_dist);
%     for dist = 1:max_dist
%         neighbours_diagonal{1, dist} = [(dist + dist * img_padded_height) ...
%             (-1 * dist + dist * img_padded_height) ...
%             (dist - 1 * dist * img_padded_height) ...
%             (-1 * dist - 1 * dist * img_padded_height)];
%         neighbours_horizontal{1, dist} =  [dist ...
%             (-1 * dist) ...
%             (dist * img_padded_height) ...
%             (-1 * dist * img_padded_height)];
%     end

    for i = 1:numel(missing_pixels_idx)
        idx = missing_pixels_idx(i);
        for dist = 1:max_dist
%             neighbour_pixels_idx = idx + neighbours_horizontal{1, dist};
            neighbour_pixels_idx = [(dist + idx) ...
                (-1 * dist + idx) ...
                (dist * img_padded_height + idx) ...
                (-1 * dist * img_padded_height + idx)];
            nnz_count = 0;
            total = 0.0;
...
%             neighbour_pixels_idx = idx + neighbours_diagonal{1, dist};
