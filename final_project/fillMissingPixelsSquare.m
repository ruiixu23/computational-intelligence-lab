...
% non-missing pixels surrounding it. It first uses all neighbours of
% distance 1 to check the non-missing pixels. If they are all missing, it checks
% surrounding pixels at distance 2 and so on. Finally, it stops
% at Manhatten distance specified by the parameter `max_distance`. Note
% that a value of 0.5 will be used to fill the missing pixels if all
% surrounding pixels of Manhattan distance up to `max_distance` are all
% missing.
%
% Input:
...
function img_filled = fillMissingPixelsSquare(img, msk, max_dist)
    ...
    for dist = 1:max_dist
        neighbour_offsets = zeros(1, 8 * dist);
        count = 1;
        for offset_idx = -1 * dist:1:dist
            neighbour_offsets(count) = offset_idx * img_padded_height + offset_idx;
            count = count + 1;
            neighbour_offsets(count) = offset_idx * img_padded_height - offset_idx;
            count = count + 1;
        end

        for position_idx = -1 * dist + 1:1:dist - 1
            neighbour_offsets(count) = -1 * dist * img_padded_height + position_idx;
            count = count + 1;
            neighbour_offsets(count) = dist * img_padded_height + position_idx;
            count = count + 1;
        end
