...
% non-missing pixels surrounding it. It first uses diamond of distance 1
% to check the non-missing pixels. If they are all missing, it checks
% surrounding pixels at distance 2 and so on. Finally, it stops at
% distance specified by the parameter `max_distance`. Note that a value
% of 0.5 will be used to fill the missing pixels if all
% surrounding pixels of distance up to `max_distance` are all
% missing.
%
% Input:
...
function img_filled = fillMissingPixelsDiamond(img, msk, max_dist)
    ...
    for dist = 1:max_dist
        neighbour_offsets = zeros(1, 4 * dist);
        count = 1;
        neighbour_offsets(1) = dist;
        neighbour_offsets(2) = -1 * dist;
        neighbour_offsets(3) = dist * img_padded_height;
        neighbour_offsets(4) = -1 * dist * img_padded_height;
        count = 5;
        for offset_idx = 1:1:dist-1
            neighbour_offsets(count) = offset_idx * img_padded_height + (dist - offset_idx);
            count = count + 1;
            neighbour_offsets(count) = offset_idx * img_padded_height - (dist - offset_idx);
            count = count + 1;
            neighbour_offsets(count) = -1 * offset_idx * img_padded_height + (dist - offset_idx);
            count = count + 1;
            neighbour_offsets(count) = -1 * offset_idx * img_padded_height - (dist - offset_idx);
            count = count + 1;
        end
        neighbours{1, dist} = neighbour_offsets;
    end
